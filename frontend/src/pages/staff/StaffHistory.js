  ...
  useEffect(() => {
    if (!userId) return;
      const fetchRequests = async () => {
        try {
          const res = await fetch(`http://localhost:5000/api/requests/staff/${userId}`);
  ...
  return (
    <div className="flex h-screen bg-gray-100 text-gray-900">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <h1 className="text-3xl font-bold text-[#002B7F] mb-6">Request History</h1>

        {requests.length === 0 ? (
          <p className="text-gray-500 italic">No request history found.</p>
        ) : (
              ...
              <tbody>
                {requests.map((req) => (
                  <tr key={req._id} className="border-b hover:bg-gray-50">
                    <td className="py-3 font-medium">{req.itemName}</td>
                    <td>{req.quantity}</td>
                    <td>
                      <span className={`px-2 py-1 rounded-md text-xs font-medium ${getStatusStyle(req.status)}`}>
                        {req.status}
                      </span>
                    </td>
